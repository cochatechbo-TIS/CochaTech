...
return new class extends Migration {
    public function up(): void {
        Schema::create('evaluacion', function (Blueprint $table) {
            ...
            $table->foreignId('id_fase')->constrained('fase', 'id_fase');
            $table->foreignId('id_olimpista')->constrained('olimpista', 'id_olimpista');
            $table->foreignId('id_evaluador')->constrained('evaluador', 'id_evaluador');
            $table->decimal('nota', 5, 2)->nullable();
            $table->decimal('nota_minima', 5, 2)->nullable();
            $table->text('comentario')->nullable();
            $table->boolean('falta_etica')->default(false);
            $table->foreignId('id_estado')->nullable()->constrained('estado', 'id_estado');
        });

        // ✅ Agregar los CHECK constraints usando DB::statement
